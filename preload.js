...
contextBridge.exposeInMainWorld('pixel', {
  ...
  ai: {
    // Renderer only ever sends/receives plain text — the key and the
    // network call both live in main.js and never reach this side.
    send: (text) => ipcRenderer.invoke('pixel:ai:send', text),
  },
});
