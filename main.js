...
// ---- Read the AI key from the environment ONLY. Never sent to the renderer. ----
...
// ---- AI: the key never leaves this process. Renderer only sends/receives text. ----
ipcMain.handle('pixel:ai:send', async (e, text) => {
      ...
      (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            resolve({ text: textOut || '(no response)' });
          } catch (err) {
